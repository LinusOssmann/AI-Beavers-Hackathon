...
import { MockSuggestionsLoader } from "./mock-suggestions-loader";
...
interface SuggestionsFeedProps {
	...
	existingGenerationStatus?: string;
}
...
export function SuggestionsFeed({
	...
	const [isPollingLocations, setIsPollingLocations] = useState(
		existingGenerationStatus === "generating_locations"
	);
	const [isPollingResearch, setIsPollingResearch] = useState(
		existingGenerationStatus === "generating_research"
	);
		...
		const pollPlan = async () => {
			try {
				...
				if (nextLocations.length > 0 && unchangedLocationPollsRef.current >= 3) {
					...
					await fetch(`/api/plans/${planId}/complete-generation`, {
						method: "POST",
	...
	const handleCreatePlan = async (data: { title: string; description?: string }) => {
		// Create a new plan
		...
		// Start location generation immediately
		await startLocationGeneration(plan.id);
	};

	const startLocationGeneration = async (targetPlanId: string) => {
	...
	const handleRegenerate = async () => {
		...
		// Start new location generation
		await startLocationGeneration(planId);
	};
					...
					<div className="max-w-[1400px] mx-auto">
						...
						<div className="flex flex-col items-center justify-center py-16">
							...
							<p className="text-muted-foreground text-center max-w-xs mb-6">
								Create a new travel plan and get personalized AI-powered suggestions.
							</p>
							<button
		...
		<>
			...
			<div className="flex-1 px-8 md:px-16 lg:px-24 xl:px-32 overflow-y-auto py-8">
				<div className="max-w-[1400px] mx-auto">
					...
					{/* Mock content while loading */}
					{isPollingLocations && locations.length === 0 && (
						<MockSuggestionsLoader />
					)}
