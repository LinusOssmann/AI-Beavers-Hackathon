...
export function Filters({ filters, onApply, onReset }: FiltersProps) {
	...
	useEffect(() => {
		const timer = setTimeout(async () => {
			if (
				...
				localFilters.destination.length >= 2
			) {
				setIsLoadingDestination(true);
				try {
					const response = await fetch(
						`/api/cities/search?q=${encodeURIComponent(
							localFilters.destination,
						)}&limit=10`,
					);
					if (response.ok) {
						const data = await response.json();
						const cityNames = data.cities.map(
	...
	useEffect(() => {
		const timer = setTimeout(async () => {
			if (
				...
				localFilters.startingPoint.length >= 2
			) {
				...
				try {
					...
					console.error(
						"Error fetching starting point cities:",
						error,
					);
					setStartingPointSuggestions([]);
				} finally {
	...
	const getActiveChips = useCallback(() => {
		...
		if (filters.destination)
			chips.push({ key: "destination", label: filters.destination });
		...
		if (filters.flexibleDates)
			chips.push({ key: "flexibleDates", label: "Flexible dates" });
		if (filters.budgetMin > 0 || filters.budgetMax < 10000) {
			chips.push({
				key: "budget",
				label: `$${filters.budgetMin} - $${filters.budgetMax}`,
			});
		}
		if (filters.numberOfTravelers > 1)
			chips.push({
				key: "travelers",
				label: `${filters.numberOfTravelers} travelers`,
			});
		if (filters.startingPoint)
			chips.push({
				key: "startingPoint",
				label: `From ${filters.startingPoint}`,
			});
		if (filters.tripDurationMin > 1 || filters.tripDurationMax < 30) {
			chips.push({
				key: "duration",
				label: `${filters.tripDurationMin}-${filters.tripDurationMax} days`,
			});
		}
		if (filters.travelingWithPet)
	...
	const moreFiltersCount =
		...
		(localFilters.tripDurationMin > 1 || localFilters.tripDurationMax < 30
			? 1
			: 0) +
	...
	const toggleStyle = (styleId: string) => {
		...
		if (current.includes(styleId)) {
			setLocalFilters({
				...localFilters,
				travelStyles: current.filter((s) => s !== styleId),
			});
		} else {
			setLocalFilters({
				...localFilters,
				travelStyles: [...current, styleId],
			});
		}
	};
	...
	const removeChip = (key: string) => {
		...
		if (key === "destination") newFilters.destination = "";
			...
			newFilters.travelStyles = newFilters.travelStyles.filter(
				(s) => s !== styleId,
			);
		}
		onApply(newFilters);
	...
	return (
		<div className="bg-background border-border sticky top-0 z-20 border-b-0">
						...
						<div className="flex items-center px-3 py-2.5 bg-card rounded-lg border border-border gap-2">
							...
							<div className="flex items-center justify-between flex-1 gap-1.5">
								<button
									...
									disabled={
										localFilters.numberOfTravelers <= 1
									}
								...
								<button
									...
									disabled={
										localFilters.numberOfTravelers >= 10
									}
									className="w-6 h-6 rounded-md bg-secondary border border-border flex items-center justify-center hover:border-primary disabled:opacity-40 disabled:cursor-not-allowed transition-all text-foreground"
								>
									<span className="text-sm">+</span>
			...
			{/* More Filters Modal */}
			{showMoreFiltersModal && (
				<div className="fixed inset-0 z-50 flex items-center justify-center mt-15 p-2 sm:p-4">
					{/* Backdrop */}
					<div
					...
					{/* Modal */}
					<div className="relative bg-card rounded-lg sm:rounded-xl w-full max-w-[95vw] sm:max-w-xl md:max-w-2xl max-h-[95vh] sm:max-h-[90vh] flex flex-col shadow-2xl">
						{/* Header */}
						<div className="flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 border-b border-border shrink-0">
							...
							<button
								onClick={() => setShowMoreFiltersModal(false)}
								className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-secondary transition-colors shrink-0"
							>
								<X className="w-5 h-5" />
							</button>
						</div>
						...
						<div className="flex-1 overflow-y-auto px-4 sm:px-6 py-4 sm:py-6">
							<div className="space-y-4 sm:space-y-6">
								{/* Starting Point */}
								<div className="relative">
									<h3 className="text-sm font-bold text-foreground mb-2 sm:mb-3">
										Starting From
									</h3>
									<div className="relative">
										...
										<input
											...
											className="w-full bg-secondary text-foreground placeholder:text-muted-foreground pl-9 pr-9 py-2 sm:py-2.5 rounded-lg border border-border focus:border-primary focus:outline-none transition-colors text-sm"
										/>
										{localFilters.startingPoint && (
											<button
												onClick={() =>
									...
									{showStartingPointDropdown &&
										(startingPointSuggestions.length > 0 ||
											...
											<div className="absolute top-full left-0 right-0 mt-1 bg-card border border-border rounded-lg shadow-xl z-30 max-h-48 overflow-y-auto">
												...
												) : startingPointSuggestions.length >
												  0 ? (
													startingPointSuggestions.map(
														(loc) => (
															<button
																key={loc}
																onMouseDown={() =>
														...
														tripDurationMin:
															Math.max(
																...
																Math.min(
																	...
																	localFilters.tripDurationMax,
																),
															),
													})
												}
												className="w-full px-2 sm:px-3 py-2 sm:py-2.5 bg-secondary border border-border rounded-lg text-sm focus:border-primary focus:outline-none transition-colors"
												placeholder="Min"
											/>
										</div>
								...
								<div>
									...
									<div className="flex flex-wrap gap-1.5 sm:gap-2">
										{TRAVEL_STYLE_OPTIONS.map((style) => {
											const isSelected =
												localFilters.travelStyles.includes(
													style.id,
												);
											return (
												<button
													...
													className={`px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-lg border text-xs sm:text-sm font-medium transition-all ${
														isSelected
								...
								<div>
									...
									<label className="flex items-center gap-2 sm:gap-3 p-2.5 sm:p-3 rounded-lg border border-border hover:bg-secondary/50 cursor-pointer transition-all">
										<input
											type="checkbox"
											checked={
												localFilters.travelingWithPet
											}
											onChange={(e) =>
												setLocalFilters({
													...localFilters,
						...
						{/* Footer */}
						<div className="flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-between gap-2 sm:gap-4 px-4 sm:px-6 py-3 sm:py-4 border-t border-border shrink-0">
							<button
								onClick={handleReset}
